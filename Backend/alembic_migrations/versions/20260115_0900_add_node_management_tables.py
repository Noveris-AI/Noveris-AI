...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('connection_type', sa.Enum('ssh', 'local', 'winrm', name='connection_type', create_type=False), default='ssh', nullable=False),
        sa.Column('ssh_user', sa.String(100)),
        sa.Column('status', sa.Enum('NEW', 'READY', 'UNREACHABLE', 'MAINTENANCE', 'DECOMMISSIONED', name='node_status', create_type=False), default='NEW', nullable=False),
        sa.Column('node_type', sa.Enum('management', 'compute', 'login', 'storage', 'kube_node', 'kube_master', 'slurm_node', 'slurm_ctrl', 'edge', 'generic', name='node_type', create_type=False), default='generic'),
    ...
    op.create_table(
        ...
        sa.Column('auth_type', sa.Enum('ssh_key', 'password', 'api_key', name='auth_type', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('bmc_protocol', sa.Enum('redfish', 'ipmi', 'none', name='bmc_protocol', create_type=False), default='redfish'),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('node_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('nodes.id', ondelete='CASCADE'), nullable=False),
        sa.Column('type', sa.Enum('nvidia_gpu', 'amd_gpu', 'intel_gpu', 'ascend_npu', 't_head_npu', 'generic_accel', name='accelerator_type', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELED', 'TIMEOUT', name='job_status', create_type=False), default='PENDING', nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
