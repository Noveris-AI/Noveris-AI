...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('framework', sa.Enum('vllm', 'sglang', 'xinference', name='deployment_framework', create_type=False), nullable=False),
        sa.Column('deployment_mode', sa.Enum('native', 'docker', name='deployment_mode', create_type=False), default='native'),
        sa.Column('node_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('nodes.id', ondelete='SET NULL'), nullable=True),
        sa.Column('model_source', sa.Enum('huggingface', 'modelscope', 'local', name='model_source', create_type=False), default='huggingface'),
        ...
        sa.Column('status', sa.Enum('PENDING', 'DOWNLOADING', 'INSTALLING', 'STARTING', 'RUNNING', 'STOPPED', 'FAILED', 'DELETING', name='deployment_status', create_type=False), default='PENDING', nullable=False),
